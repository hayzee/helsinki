...
;; (def a-seq [9 9 9 9 8 7 6 5 9])

;; (def mapped-seq (map vector a-seq (cons (first a-seq) a-seq)))

;; (def mapped-fn (if (>= (second a-seq) (first a-seq)) >= <=))

;; (def retained-seq (take-while (fn [[e1 e2]] (mapped-fn e1 e2)) mapped-seq))

;; retained-seq

;; (def rest-seq (drop-while (fn [[e1 e2]] (mapped-fn e1 e2)) mapped-seq))

;; rest-seq

;; (def result (list (map first retained-seq) (map first rest-seq)))
...
;; (defn split-into-monotonics [a-seq]
;;   (loop [accum             (list)
;;          current-monotonic (list)
;;          last-value        nil
;;          a-seq             a-seq]
;;     (if (empty? a-seq)
;;       accum
;;       (if (or (nil? last-value) (<= last-value (first a-seq)))
;;         (recur accum                          (cons (first a-seq) current-monotonic) (first a-seq) (rest a-seq))
;;         (recur (cons current-monotonic accum) (list (first a-seq))                   (first a-seq) (rest a-seq))))))
...
;;       (if (< (first s) prev)
;;         [accum s]
;;         (recur (rest s) (first s) (conj accum (first s)))))))

;; (monoton '(1 -1))


;; (defn monotonic [accum a-seq prev]
;;   (let [elem (first a-seq)]
;;     (if (or (empty? a-seq) (< elem (or prev elem)))
;;       (list (reverse accum) a-seq)
;;       (monotonic (cons elem accum) (rest a-seq) elem))))

;; (monotonic '() [1 1 1 2 2 1] nil)


;; (defn split-into-monotonics [a-seq]
;;   (if (empty? a-seq)
;;     (list)
;;     (let [[h t] (monoton a-seq)]
;;       (cons h (split-into-monotonics t)))))
